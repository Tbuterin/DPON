...
def get_dpo_prompt(env_name, obs, admissible_actions, history="", action_only = False):
    ...
    if not action_only:
        if len(history) > 0:
            ...
            qs = qs + f"Here are your history steps: \n"
            qs = qs + f"[History]\n{history}\n[end History]\n"
            ...
            # qs = qs + "\"reflections\": \"{reflect on your historical trajectory and carefully think about which action to complete the task.}\", \n"
            qs = qs + "\"action\": \"{an admissible action}\"\n\}"  # jkc0923
            # qs = qs + "your output need to be in 60 words!\n"  # jkc0924
        else:
            refomratted_admissible_actions = "\n ".join(f"'{s}'" for s in admissible_actions)
            qs = f"Your are an expert in the ALFRED Embodied Environment. "
            qs = qs + f"Your task is to " + task + ". "
            ...
            qs = qs + "Your response should be a valid json file in the following format: \n\{\n"
            qs = qs + "\"thoughts\": \"{first describe what do you see in the image using the text description, then carefully think about which action to complete the task. }\", \n"
            # qs = qs + "\"reflections\": \"{describe what do you see in the image using the text description in 20 words, then carefully think about which action to complete the task.}\", \n"
            qs = qs + "\"action\": \"{an admissible action}\"\n\}"  # jkc0923
            # qs = qs + "your output need to be in 60 words!\n"  # jkc0924
    else:
        refomratted_admissible_actions = "\n ".join(f"'{s}'" for s in admissible_actions)
